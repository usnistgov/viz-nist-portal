...
tests.forEach(function (test) {
  test('works', function (dns, t) {
    ...
    dns.query('viz-nist-portal', function () {
      t.ok(true, 'flushed')
    })
  })
  ...
  test('ANY query', function (dns, t) {
    dns.once('query', function (packet) {
      ...
      t.same(packet.questions[0], {name: 'viz-nist-portal', type: 'ANY', class: 'IN'})
      dns.destroy(function () {
        t.end()
      })
    })

    dns.query('viz-nist-portal', 'ANY')
  })

  test('A record', function (dns, t) {
    dns.once('query', function (packet) {
      t.same(packet.questions.length, 1, 'one question')
      t.same(packet.questions[0], {name: 'viz-nist-portal', type: 'A', class: 'IN'})
      dns.respond([{type: 'A', name: 'viz-nist-portal', ttl: 120, data: '127.0.0.1'}])
    })

    dns.once('response', function (packet) {
      t.same(packet.answers.length, 1, 'one answer')
      t.same(packet.answers[0], {type: 'A', name: 'viz-nist-portal', ttl: 120, data: '127.0.0.1', class: 'IN', flush: false})
      dns.destroy(function () {
        t.end()
      })
    })

    dns.query('viz-nist-portal', 'A')
  })
  ...
  test('A record (two questions)', function (dns, t) {
    ...
    dns.query([{name: 'viz-nist-portal', type: 'A'}, {name: 'hej.verden', type: 'A'}])
  })

  test('AAAA record', function (dns, t) {
    dns.once('query', function (packet) {
      t.same(packet.questions.length, 1, 'one question')
      t.same(packet.questions[0], {name: 'viz-nist-portal', type: 'AAAA', class: 'IN'})
      dns.respond([{type: 'AAAA', name: 'viz-nist-portal', ttl: 120, data: 'fe80::5ef9:38ff:fe8c:ceaa'}])
    })
    ...
    dns.query('viz-nist-portal', 'AAAA')
  })

  test('SRV record', function (dns, t) {
    dns.once('query', function (packet) {
      t.same(packet.questions.length, 1, 'one question')
      t.same(packet.questions[0], {name: 'viz-nist-portal', type: 'SRV', class: 'IN'})
      dns.respond([{
        type: 'SRV',
        name: 'viz-nist-portal',
        ttl: 120,
  ...
  test('TXT record', function (dns, t) {
    ...
    dns.once('query', function (packet) {
      t.same(packet.questions.length, 1, 'one question')
      t.same(packet.questions[0], {name: 'viz-nist-portal', type: 'TXT', class: 'IN'})
      dns.respond([{type: 'TXT', name: 'viz-nist-portal', ttl: 120, data: data}])
    })

    dns.once('response', function (packet) {
      t.same(packet.answers.length, 1, 'one answer')
      t.same(packet.answers[0], {type: 'TXT', name: 'viz-nist-portal', ttl: 120, data: data, class: 'IN', flush: false})
      dns.destroy(function () {
        t.end()
      })
    })

    dns.query('viz-nist-portal', 'TXT')
  })
